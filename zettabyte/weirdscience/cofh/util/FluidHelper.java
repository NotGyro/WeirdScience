package zettabyte.weirdscience.cofh.util;
...
public class FluidHelper {
	...
	public static final FluidStack WATER = new FluidStack(FluidRegistry.WATER,
			FluidContainerRegistry.BUCKET_VOLUME);
	public static final FluidStack LAVA = new FluidStack(FluidRegistry.LAVA,
			FluidContainerRegistry.BUCKET_VOLUME);
	...
	public static FluidStack extractFluidFromHeldContainer(EntityPlayer player,
			int maxDrain, boolean doDrain) {

		ItemStack container = player.getCurrentEquippedItem();

		return isFluidContainerItem(container) ? ((IFluidContainerItem) container
				.getItem()).drain(container, maxDrain, doDrain) : null;
	}

	public static int insertFluidIntoHeldContainer(EntityPlayer player,
			FluidStack resource, boolean doFill) {

		ItemStack container = player.getCurrentEquippedItem();

		return isFluidContainerItem(container) ? ((IFluidContainerItem) container
				.getItem()).fill(container, resource, doFill) : 0;
	}
	...
	public static ItemStack setDefaultFluidTag(ItemStack container,
			FluidStack resource) {
	...
	public static FluidStack extractFluidFromAdjacentFluidHandler(
			TileEntity tile, int side, int maxDrain, boolean doDrain) {

		TileEntity handler = BlockHelper.getAdjacentTileEntity(tile, side);

		return handler instanceof IFluidHandler ? ((IFluidHandler) handler)
				.drain(ForgeDirection.VALID_DIRECTIONS[side ^ 1], maxDrain,
						doDrain) : null;
	}

	public static int insertFluidIntoAdjacentFluidHandler(TileEntity tile,
			int side, FluidStack fluid, boolean doFill) {

		TileEntity handler = BlockHelper.getAdjacentTileEntity(tile, side);

		return handler instanceof IFluidHandler ? ((IFluidHandler) handler)
				.fill(ForgeDirection.VALID_DIRECTIONS[side ^ 1], fluid, doFill)
				: 0;
	}
	...
	public static boolean fillContainerFromHandler(World world,
			IFluidHandler handler, EntityPlayer player, FluidStack tankFluid) {

		ItemStack container = player.getCurrentEquippedItem();

		if (FluidContainerRegistry.isEmptyContainer(container)) {
			ItemStack returnStack = FluidContainerRegistry.fillFluidContainer(
					tankFluid, container);
	...
	public static boolean fillHandlerWithContainer(World world,
			...
			if (handler.fill(ForgeDirection.UNKNOWN, fluid, false) == fluid.amount
				...
				if (!player.capabilities.isCreativeMode) {
					player.inventory.setInventorySlotContents(
							...
							ItemHelper.consumeItem(container));
				}
				return true;
			}
		}
	...
	/* PACKETS */
	public static void writeFluidStackToPacket(FluidStack fluid, DataOutput data)
			throws IOException {

		if (!isValidFluidStack(fluid)) {
			data.writeShort(-1);
		} else {
			byte[] abyte = CompressedStreamTools.compress(fluid
	...
	public static FluidStack readFluidStackFromPacket(DataInput data)
			throws IOException {
		...
		if (length < 0) {
			...
			return FluidStack.loadFluidStackFromNBT(CompressedStreamTools
					.decompress(abyte));
		}
	}
	...
	public static boolean isValidFluidStack(FluidStack fluid) {
		...
		return fluid == null ? false : fluid.fluidID == 0 ? false
				: FluidRegistry.getFluidName(fluid) != null;
	}
	...
	public static FluidStack getFluidFromWorld(World worldObj, int x, int y,
			int z) {
		...
		} else if (Block.blocksList[bId] != null
				&& Block.blocksList[bId] instanceof IFluidBlock) {
	...
	public static FluidStack getFluidForFilledItem(ItemStack container) {
		...
		if (container != null
				&& container.getItem() instanceof IFluidContainerItem) {
			return ((IFluidContainerItem) container.getItem())
					.getFluid(container);
		}
		return FluidContainerRegistry.getFluidForFilledItem(container);
	}

	public static boolean isFluidEqualOrNull(FluidStack resourceA,
			FluidStack resourceB) {

		return resourceA == null || resourceB == null
				|| resourceA.isFluidEqual(resourceB);
	}

	public static boolean isFluidEqualOrNull(Fluid fluidA, FluidStack resourceB) {

		return fluidA == null || resourceB == null
				|| fluidA == resourceB.getFluid();
	}
	...
	public static boolean isFluidEqual(FluidStack resourceA,
			FluidStack resourceB) {
