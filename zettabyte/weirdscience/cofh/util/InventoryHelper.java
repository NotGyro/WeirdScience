...
public class InventoryHelper {
	...
	public static boolean addItemStackToInventory(ItemStack[] inventory,
		...
		for (int i = startIndex; i < inventory.length; i++) {
			if (ItemHelper.areItemStacksEqualNoNBT(stack, inventory[i])
	...
	public static ItemStack extractItemStackFromInventory(
			IInventory theInventory, int side) {
		...
		if (theInventory instanceof ISidedInventory) {
			...
			for (int i = 0; i < theInventory.getSizeInventory()
					&& retStack == null; i++) {
	...
	public static ItemStack insertItemStackIntoInventory(
			IInventory theInventory, ItemStack stack, int side) {
		...
		if (theInventory instanceof ISidedInventory) {
			...
			for (int i = 0; i < slots.length && stack != null; i++) {
				if (sidedInv.canInsertItem(slots[i], stack, side)
						&& ItemHelper.itemsEqualWithMetadata(stack,
								theInventory.getStackInSlot(slots[i]), true)) {
			...
			for (int i = 0; i < slots.length && stack != null; i++) {
				if (sidedInv.canInsertItem(slots[i], stack, side)
						&& theInventory.getStackInSlot(slots[i]) == null) {
			...
			for (int i = 0; i < invSize && stack != null; i++) {
				if (ItemHelper.itemsEqualWithMetadata(stack,
						theInventory.getStackInSlot(i), true)) {
	...
	public static ItemStack simulateInsertItemStackIntoInventory(
		...
		if (theInventory instanceof ISidedInventory) {
			...
			for (int i = 0; i < invSize && stack != null; i++) {
				if (ItemHelper.itemsEqualWithMetadata(stack,
						theInventory.getStackInSlot(i), true)) {
					stack = simulateAddToOccupiedInventorySlot(theInventory, i,
							stack);
				}
			}
			for (int i = 0; i < invSize && stack != null; i++) {
				if (theInventory.getStackInSlot(i) == null) {
					stack = simulateAddToEmptyInventorySlot(theInventory, i,
							stack);
				}
			}
		}
	...
	/* Slot Interaction */
	public static ItemStack addToEmptyInventorySlot(IInventory theInventory,
		...
		theInventory.setInventorySlotContents(
				slot,
	...
	public static ItemStack addToOccupiedInventorySlot(IInventory theInventory,
			int slot, ItemStack stack) {

		ItemStack stackInSlot = theInventory.getStackInSlot(slot);
		int stackLimit = Math.min(theInventory.getInventoryStackLimit(),
	...
	public static ItemStack simulateAddToEmptyInventorySlot(
			IInventory theInventory, int slot, ItemStack stack) {
	...
	/* HELPERS */
	public static ItemStack addToInsertion(Object theTile, int side,
			ItemStack stack) {
		...
		if (theTile instanceof IInventory) {
			stack = insertItemStackIntoInventory((IInventory) theTile, stack,
					BlockHelper.SIDE_OPPOSITE[side]);
		} else {
			stack = ((IItemConduit) theTile).insertItem(
