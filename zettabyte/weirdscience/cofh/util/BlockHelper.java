...
public final class BlockHelper {
	...
	public static float[][] SIDE_COORD_AABB = { { 1, -2, 1 }, { 1, 2, 1 },
	...
	public static MovingObjectPosition getCurrentMovingObjectPosition(
		...
		Vec3 posVec = Vec3.createVectorHelper(player.posX, player.posY,
				player.posZ);
		Vec3 lookVec = player.getLook(1);
		posVec.yCoord += player.getEyeHeight();
		lookVec = posVec.addVector(lookVec.xCoord * distance, lookVec.yCoord
				* distance, lookVec.zCoord * distance);
	...
	public static TileEntity getAdjacentTileEntity(World world, int x, int y,
			int z, ForgeDirection dir) {

		return world == null ? null : world.getBlockTileEntity(x + dir.offsetX,
	...
	public static TileEntity getAdjacentTileEntity(World world, int x, int y,
		...
		return world == null ? null : getAdjacentTileEntity(world, x, y, z,
				ForgeDirection.values()[side]);
	}

	public static TileEntity getAdjacentTileEntity(TileEntity refTile,
			ForgeDirection dir) {

		return refTile == null ? null : getAdjacentTileEntity(refTile.worldObj,
				refTile.xCoord, refTile.yCoord, refTile.zCoord, dir);
	}

	public static TileEntity getAdjacentTileEntity(TileEntity refTile, int side) {

		return refTile == null ? null : getAdjacentTileEntity(refTile.worldObj,
				refTile.xCoord, refTile.yCoord, refTile.zCoord,
				ForgeDirection.values()[side]);
	}

	public static int determineAdjacentSide(TileEntity refTile, int x, int y,
			int z) {

		return y > refTile.yCoord ? 1 : y < refTile.yCoord ? 0
				: z > refTile.zCoord ? 3 : z < refTile.zCoord ? 2
						: x > refTile.xCoord ? 5 : 4;
	}
	...
	public static AxisAlignedBB getAdjacentAABBForSide(int x, int y, int z,
			int side) {

		return AxisAlignedBB.getAABBPool().getAABB(x + SIDE_COORD_MOD[side][0],
				y + SIDE_COORD_MOD[side][1], z + SIDE_COORD_MOD[side][2],
				x + SIDE_COORD_AABB[side][0], y + SIDE_COORD_AABB[side][1],
	...
	public static int rotateVanillaBlock(World world, int bId, int bMeta,
			int x, int y, int z) {
		...
		case RotationType.CHEST:
			...
			for (int i = 2; i < 6; i++) {
				...
				if (world.getBlockId(coords[0], coords[1], coords[2]) == Block.chest.blockID) {
					world.setBlockMetadataWithNotify(coords[0], coords[1],
							coords[2], SIDE_OPPOSITE[bMeta], 1);
	...
	public static int rotateVanillaBlockAlt(World world, int bId, int bMeta,
			int x, int y, int z) {
	...
	public static boolean isBlock(int bId) {
		...
		return bId < 0 || bId >= Block.blocksList.length ? null
				: Block.blocksList[bId] != null;
	}

	public static List<ItemStack> breakBlock(World worldObj, int x, int y,
			int z, int bId, int fortune, boolean doBreak, boolean silkTouch) {
		...
		if (silkTouch
				&& Block.blocksList[bId].canSilkHarvest(worldObj, null, x, y,
						z, meta)) {
			ItemStack itemstack = createStackedBlock(Block.blocksList[bId],
					meta);
		} else {
			stacks = Block.blocksList[bId].getBlockDropped(worldObj, x, y, z,
					meta, fortune);
		}
		...
		List result = worldObj.getEntitiesWithinAABB(EntityItem.class,
				AxisAlignedBB.getBoundingBox(x - 2, y - 2, z - 2, x + 3, y + 3,
						z + 3));
