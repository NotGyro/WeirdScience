...
public final class ItemHelper {
	...
	public static final boolean isPlayerHoldingItem(Item item,
		...
		Item equipped = player.getCurrentEquippedItem() != null ? player
	...
	public static final boolean isPlayerHoldingItemStack(ItemStack stack,
		...
		ItemStack equipped = player.getCurrentEquippedItem() != null ? player
	...
	public static final boolean areItemStacksEqual(ItemStack stackA,
			ItemStack stackB) {

		return ItemStack.areItemStacksEqual(stackA, stackB);
	}

	public static final boolean areItemStackEqualNoNull(ItemStack stackA,
			ItemStack stackB) {

		return stackA.isItemEqual(stackB)
				&& (stackA.stackTagCompound != null ? stackB.stackTagCompound != null
						&& stackA.stackTagCompound
								.equals(stackB.stackTagCompound)
						: stackB.stackTagCompound != null ? false : true);
	}

	public static boolean areItemStacksEqualNoNBT(ItemStack stackA,
			ItemStack stackB) {
	...
	public static boolean craftingEquivalent(ItemStack checked,
		...
		return output == null ? areItemStacksEqualNoNBT(checked, source) ? true
				: oreDict == null ? false : oreDict.equals("Unknown") ? false
						: getOreName(checked).equalsIgnoreCase(oreDict)
				: isBlacklist(output) ? areItemStacksEqualNoNBT(checked, source)
						: areItemStacksEqualNoNBT(checked, source) ? true
								: oreDict == null ? false : oreDict
										.equals("Unknown") ? false
										: getOreName(checked).equalsIgnoreCase(
												oreDict);
	}

	public static boolean isBlacklist(ItemStack output) {

		return output.itemID == Block.stairsWoodBirch.blockID
				|| output.itemID == Block.stairsWoodJungle.blockID
				|| output.itemID == Block.stairsWoodOak.blockID
				|| output.itemID == Block.stairsWoodSpruce.blockID
				|| output.itemID == Block.planks.blockID
	...
	public static String getItemNBTString(ItemStack theItem, String nbtKey,
		...
		return theItem.stackTagCompound != null ? theItem.stackTagCompound
				.hasKey(nbtKey) ? theItem.stackTagCompound.getString(nbtKey)
	...
	public static boolean itemsEqualWithMetadata(ItemStack stackA,
		...
		return stackA == null ? stackB == null ? true : false
				: stackB != null
						&& stackA.itemID == stackB.itemID
						&& (stackA.getHasSubtypes() == false || stackA
								.getItemDamage() == stackB.getItemDamage());
	}

	public static boolean itemsEqualWithoutMetadata(ItemStack stackA,
			ItemStack stackB) {

		return stackA == null ? stackB == null ? true : false : stackB != null
				&& stackA.itemID == stackB.itemID;
	}
	...
	public static boolean doOreIDsMatch(ItemStack stackA, ItemStack stackB) {
		...
		return OreDictionary.getOreID(stackA) >= 0
				&& OreDictionary.getOreID(stackA) == OreDictionary
						.getOreID(stackB);
	}

	public static boolean doNBTsMatch(NBTTagCompound nbtA, NBTTagCompound nbtB) {

		return nbtA == null ? nbtB == null ? true : false
				: nbtB == null ? false : nbtA.equals(nbtB);
	}
	...
	public static void addInventoryInformation(ItemStack stack, List list,
		...
		if (stack.stackTagCompound.hasKey("Inventory")
				&& stack.stackTagCompound.getTagList("Inventory").tagCount() > 0) {
			...
			list.add(StringHelper.localize("info.zettabyte.weirdscience.cofh.contents") + ":");
			...
			for (ItemStack item : containedItems) {
				...
				if (!StringHelper.displayStackCount
					...
					list.add("    " + StringHelper.BRIGHT_GREEN
							+ item.stackSize + " " + StringHelper.GRAY
							+ item.getDisplayName());
				} else {
					if (item.stackSize % maxStackSize != 0) {
						list.add("    " + StringHelper.BRIGHT_GREEN
								+ maxStackSize + "x" + item.stackSize
								/ maxStackSize + "+" + item.stackSize
								% maxStackSize + " " + StringHelper.GRAY
								+ item.getDisplayName());
					} else {
						list.add("    " + StringHelper.BRIGHT_GREEN
								+ maxStackSize + "x" + item.stackSize
