...
import zettabyte.weirdscience.cofh.gui.element.ElementBase;
import zettabyte.weirdscience.cofh.gui.element.TabBase;
import zettabyte.weirdscience.cofh.gui.slot.SlotFalseCopy;
import zettabyte.weirdscience.cofh.render.RenderHelper;
import zettabyte.weirdscience.cofh.util.StringHelper;
...
public abstract class GuiBase extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int x, int y) {
		...
		fontRenderer.drawString(StringHelper.localize(name),
				getCenteredOffset(StringHelper.localize(name)), 6, 0x404040);
		if (drawInventory) {
	...
	//Mojang don't like polymorphism.
    protected Slot getSlotAtPosition(int x, int y) {
        for (int iter = 0; iter < this.inventorySlots.inventorySlots.size(); iter++) {
            Slot slot = (Slot)this.inventorySlots.inventorySlots.get(iter);
	...
	@Override
	protected void mouseClickMove(int mX, int mY, int lastClick,
			long timeSinceClick) {

		Slot slot = this.getSlotAtPosition(mX, mY);
		ItemStack itemstack = this.mc.thePlayer.inventory.getItemStack();

		if (this.field_94076_q && slot != null && itemstack != null
	 ...
	 */
	//@Override
	public List<String> handleTooltip(int mousex, int mousey,
			List<String> tooltip) {
...
/*
	public void drawButton(String iconName, int x, int y, int spriteSheet,
			int mode) {
	...
	public void drawTiledTexture(int x, int y, Icon icon, int width, int height) {
		...
		for (i = 0; i < width; i += 16) {
			for (j = 0; j < height; j += 16) {
				...
				drawScaledTexturedModelRectFromIcon(x + i, y + j, icon,
...
/*
	...
	public void drawSizedTexturedModalRect(int x, int y, int u, int v,
			int width, int height, float texW, float texH) {

		float texU = 1 / texW;
		float texV = 1 / texH;
		Tessellator tessellator = Tessellator.instance;
		tessellator.startDrawingQuads();
		tessellator.addVertexWithUV(x + 0, y + height, this.zLevel, (u + 0)
				* texU, (v + height) * texV);
		tessellator.addVertexWithUV(x + width, y + height, this.zLevel,
				(u + width) * texU, (v + height) * texV);
		tessellator.addVertexWithUV(x + width, y + 0, this.zLevel, (u + width)
				* texU, (v + 0) * texV);
		tessellator.addVertexWithUV(x + 0, y + 0, this.zLevel, (u + 0) * texU,
				(v + 0) * texV);
		tessellator.draw();
	}

	public void drawScaledTexturedModelRectFromIcon(int x, int y, Icon icon,
		...
		Tessellator tessellator = Tessellator.instance;
		tessellator.startDrawingQuads();
		tessellator.addVertexWithUV(x + 0, y + height, this.zLevel, minU, minV
				+ (maxV - minV) * height / 16F);
		tessellator.addVertexWithUV(x + width, y + height, this.zLevel, minU
				+ (maxU - minU) * width / 16F, minV + (maxV - minV) * height
				/ 16F);
		tessellator.addVertexWithUV(x + width, y + 0, this.zLevel, minU
	...
	protected void drawTooltipHoveringText(List list, int x, int y,
			FontRenderer font) {
		...
		this.drawGradientRect(i1 - 3, j1 + k1 + 3, i1 + k + 3, j1 + k1 + 4, l1,
				l1);
		this.drawGradientRect(i1 - 3, j1 - 3, i1 + k + 3, j1 + k1 + 3, l1, l1);
		this.drawGradientRect(i1 - 4, j1 - 3, i1 - 3, j1 + k1 + 3, l1, l1);
		this.drawGradientRect(i1 + k + 3, j1 - 3, i1 + k + 4, j1 + k1 + 3, l1,
				l1);
		int i2 = 1347420415;
		int j2 = (i2 & 16711422) >> 1 | i2 & -16777216;
		this.drawGradientRect(i1 - 3, j1 - 3 + 1, i1 - 3 + 1, j1 + k1 + 3 - 1,
				i2, j2);
		this.drawGradientRect(i1 + k + 2, j1 - 3 + 1, i1 + k + 3, j1 + k1 + 3
				- 1, i2, j2);
		this.drawGradientRect(i1 - 3, j1 - 3, i1 + k + 3, j1 - 3 + 1, i2, i2);
		this.drawGradientRect(i1 - 3, j1 + k1 + 2, i1 + k + 3, j1 + k1 + 3, j2,
				j2);
