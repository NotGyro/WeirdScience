package zettabyte.weirdscience.cofh.world.feature;
...
import zettabyte.weirdscience.cofh.api.world.WeightedRandomBlock;
...
public class WorldGenMinableCluster extends WorldGenerator {
	...
	public WorldGenMinableCluster(List<WeightedRandomBlock> resource,
			int clusterSize) {
	...
	public WorldGenMinableCluster(WeightedRandomBlock resource,
			int clusterSize, int blockID) {
	...
	public boolean generate(World world, Random rand, int x, int y, int z) {
		...
		for (int l = 0; l <= genClusterSize; ++l) {
			...
			double d10 = (MathHelper.sin(l * (float) Math.PI / genClusterSize) + 1.0F)
					* d9 + 1.0D;
			double d11 = (MathHelper.sin(l * (float) Math.PI / genClusterSize) + 1.0F)
					* d9 + 1.0D;
					...
					for (int l2 = j1; l2 <= i2; ++l2) {
						...
						if (d12 * d12 + d13 * d13 < 1.0D) {
							for (int i3 = k1; i3 <= j2; ++i3) {
								...
								Block block = Block.blocksList[world
										.getBlockId(k2, l2, i3)];

								if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D
										&& block != null
										&& block.isGenMineableReplaceable(
												world, k2, l2, i3, genBlockID)) {
									WeightedRandomBlock ore = (WeightedRandomBlock) WeightedRandom
											.getRandomItem(world.rand, cluster);
									world.setBlock(k2, l2, i3, ore.blockId,
											ore.metadata, 0);
								}
							}
						}
