...
public class BlockFluidReactive extends BlockFluidClassicWS implements
	...
	protected ArrayList<IReactionSpec> Reactions = new ArrayList<IReactionSpec>(
			12);
	...
	public boolean registerReaction(IReactionSpec react) {
		if (react.getSolvent().getName()
				.contentEquals(this.getFluid().getName())) {
			Reactions.add(react);
								...
								if (1000 >= Reactions.get(i).getSolventMin()) {
									...
									if (itemStack.stackSize >= Reactions.get(i).getSoluteMin()) {
										...
										if (Reactions.get(i)
												.isSolventAffected()) {
											if (Reactions.get(i)
													.getSolventTarget() != null) {
												// Try to do the item thing.
												if (Reactions.get(i)
														.getSolventTarget() instanceof ItemStack) {
													ItemStack newItemStack = ((ItemStack) Reactions
															.get(i)
															.getSolventTarget())
															.copy();
												...
												// Try to do the block thing.
													...
													if (Reactions.get(i)
															.getSolventTarget() instanceof Block) {
														Block newBlock = (Block) Reactions
																.get(i)
																.getSolventTarget();
														world.setBlock(
																x,
																...
																z,
																newBlock.blockID);
													} else if (Reactions.get(i)
															.getSolventTarget() instanceof Fluid) {
														// If it 's a fluid we
														// still try to do the
														// block thing.
														int newBlock = ((Fluid) Reactions
																.get(i)
																.getSolventTarget())
																.getBlockID();
